...
( async () => {	
	...
	if(args_mine.freeforms){
		...
		try {
			...
			non_can_tags= await mine_non_canonicals(URLs.site,page,args_mine.verbose);		
		} catch (error) {
			console.error(error);
		}
		finally{
			fs.writeFile(args_mine.output_path+args_mine.output_file+".txt", non_can_tags.toString(),function(err, result) {
