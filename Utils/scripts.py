...
def get_G_info(G:nx.DiGraph, g_name='name'):
    info_dic={}
    ...
    generate_report(info_dic,'testing_report')

def get_G_diff_H_info(G:nx.DiGraph, H:nx.DiGraph, g_name='G_name', h_name='H_name'):
    ...
    G_list_edges=[edge for edge in G.edges if not H.has_edge(edge[0], edge[1])]
...
def generate_report(info_dic:dict, file_name='report'):
